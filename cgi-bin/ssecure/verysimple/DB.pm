# ----------------------------------------------------------------------------
# verysimple::DB (verysimple database) Module
...
sub xml {
	...
	my ($this) = shift;
	my ($strRootName) = shift || "verysimple::DB";
...
#_____________________________________________________________________________
sub NextId {
	# returns the next available id for a numeric field
	my $this = shift;
	my $fieldName = shift || return 0;
	my ($newId) = $this->Max($fieldName) || 0;
	return int($newId) + 1;
}
...
sub Filter {
	...
	while ($this->{'absolutePosition'} != $absolutePosition) {
		...
		if ($filterOr && !$this->{'filterArray'}[$absolutePosition]) {
			...
			$filterSetting = 1;
		} elsif ($operator eq "reg" && !($this->FieldValue($fieldName) =~ /$criteria/i)) {
			$filterSetting = 1;
...
sub Sort {
	...
	my (@sortedArray) = sort { 
		...
		if ($sortMode eq "1") {
		    ...
		    # numeric ascending
			return 0 if ($bVals[$fieldNumber] == $aVals[$fieldNumber]);
			return 1 if ($bVals[$fieldNumber] < $aVals[$fieldNumber]);
			return -1 if ($bVals[$fieldNumber] > $aVals[$fieldNumber]);
		} elsif ($sortMode eq "3") {
		    # numeric descending
			return 0 if ($bVals[$fieldNumber] == $aVals[$fieldNumber]);
...
verysimple::DB - Simple interface to text-delimited data files
...
=head1 SYNOPSIS
	...
	# create the object
	my (objDB) = new verysimple::DB(File=>'C:\\datafile.txt', Delimiter=>'\t');
...
verysimple::DB provides a simple object-oriented interface for delimited
text files.  The object model is based off of Microsoft's
ADO RecordSet object, so anyone familiar with this will
find verysimple::DB somewhat familiar.  verysimple::DB has been tested on Win32 and
Linux.
...
CR is a character or string that verysimple::DB uses to replace a Carriage Return
...
that you create another verysimple::DB object instead.
...
LF is a character or string that verysimple::DB uses to replace a Line Feed
...
optional.  Default values are "verysimple::DB" and "Record"
...
In theory this would close the file, however verysimple::DB does not keep the file handle
...
in case verysimple::DB is later modified to keep the file handle open.  This might be
...
filter.  strCriteria indicates the search term that you wish to find.

strOperator indicates how the filter comparison should work and must be
one of the following:
	"eq"   = the field value is equal to strCriteria 
	"ne"   = the field value is not equal to strCriteria 
	"reg"  = the field value matches regular expression pattern strCriteria 
	"like" = the field value contains strCriteria (case insensitive)
	"<"    = the field value is less than strCriteria 
	">"    = the field value is greater than strCriteria 
...
"OR" filtering will be used instead of AND.
	...
	# to get records containing foo AND phoo:

	$objDB->Filter("NAME", "like", "foo");
	$objDB->Filter("NAME", "like", "phoo");

	# if you wanted records containing foo OR phoo:

	$objDB->Filter("NAME", "like", "foo");
	$objDB->Filter("NAME", "like", "phoo", 1);  # (OR specified)
...
=head2 NextId(IdFieldName)

Returns the Max value + 1 for IdFieldName.  If there are no existing records, returns 1.
This is a helper method for obtaining the next available ID number when you are using
a field as a primary key.
...
=head1 VERSION HISTORY
	...
	2.100: Added type "reg" to Filter to use regular expression filtering
	2.012: Updated POD only
	2.011: Changing File property sets dirty bit
	2.010: Added IsOpen property
	2.006: Fixed bug in numeric sorting
	2.005: NameSpace changed to verysimple::DB.  Added NextId method
...
verysimple::DB loads the entire datafile into an array which could cause performance
