...
$verysimple::LockFile::LOCKER = undef;	# Default locking object
...
sub lock {
	...
	unless (ref $self) {			# Not invoked as a method
		unshift(@_, $self);
		$self = $verysimple::LockFile::LOCKER ||
			verysimple::LockFile->make('-warn' => 1);
	}
...
verysimple::LockFile - simple file locking scheme

=head1 SYNOPSIS

 use verysimple::LockFile qw(lock trylock unlock);
 ...
 $lockmgr = new verysimple::LockFile(-format => '%f.lck',
...
Each of those configuration attributes can be queried on the object directly:
	...
	$obj = verysimple::LockFile->make(-nfs => 1);
	$on_nfs = $obj->nfs;
